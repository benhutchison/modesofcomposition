...
trait ErrorValueSupport {
  ...
  class ErrorValueFromEitherPartiallyApplied[F[_]] {
    ...
    = errorValueFromEitherF(e.pure[F])
  }

  /**Convert an F[Either[E, A]] into an F[A] by raising the E if required. Requires a MonadError error effect in F. */
  def errorValueFromEitherF[F[_], A, E: ClassTag](e: F[Either[E, A]], stackTrace: Boolean = false)(
    implicit F: MonadError[F, Throwable]): F[A] =
    e.map(_.leftMap(new ErrorValue(_, stackTrace))).flatMap(F.fromEither[A] _)
